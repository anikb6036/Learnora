...
export default async function handler(req: any, res: any) {
  ...
  if (hash) {
     ...
     // Since this is stateless API on Vercel, memory store is not reliable between requests.
     // If hash is missing, we must fail.
     return res.status(400).json({ error: "No verification hash provided." });
  }
}
